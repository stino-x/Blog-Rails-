...
RSpec.describe Comment, type: :model do
  ...
  let(:post) { Post.create(title: 'Post Title', text: 'Post content', author:) }

  it 'belongs to a post' do
    comment = Comment.new(text: 'Comment text', user: author, post:)
    expect(comment.post).to eq(post)
  end

  it 'increments the post comments counter after creation' do
    expect do
      Comment.create(text: 'Comment text', user: author, post:)
      post.reload
    end.to change { post.comments_counter }.by(1)
  end

  it 'decrements the post comments counter after destruction' do
    comment = Comment.create(text: 'Comment text', user: author, post:)
    expect do
      comment.destroy
      post.reload
    end.to change { post.comments_counter }.by(-1)
  ...
  it 'validates presence of text' do
    comment = Comment.new(user: author, post:, text: nil)
    comment.valid?
